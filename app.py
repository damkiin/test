...
data_scatter = go.Scatter(x = data_count.index, y = y_rolling_avg_14, name = '14-day rolling average of COVID-19 cases related to '+phu)  
layout = go.Layout(title= 'Covid-19 disease trend for '+ str(phu),
              yaxis=dict(title = 'Cumulative frequency', color = 'blue', showgrid = True, rangemode= 'nonnegative'),
...
def rolling(phu):
    y_rolling_avg_14 = data_count[phu].rolling(14).mean()
    y_cumul = data_count[phu].cumsum()
    data_scatter2 = go.Scatter(x = data_count.index, y = y_rolling_avg_14, name = '14-day rolling average of COVID-19 cases related to '+phu)  
    layout2 = go.Layout(title= 'Covid-19 disease trend for '+ str(phu),
                  yaxis=dict(title = 'Cumulative frequency', color = 'blue', showgrid = True, rangemode= 'nonnegative'),
                   legend= dict(orientation = 'h')
                  )
...
options = [
            ...
            {'label': 'Kingston, Frontenac and Lennox & Addington Public Health', 'value': 'Kingston, Frontenac and Lennox & Addington Public Health'},
            {'label': 'Lambton Public Health', 'value': 'Lambton Public Health'},
            {'label': 'Leeds, Grenville and Lanark District Health Unit', 'value': 'Leeds, Grenville and Lanark District Health Unit'},
            {'label': 'Hastings and Prince Edward Counties Health Unit', 'value': 'Hastings and Prince Edward Counties Health Unit'},
            ...
            {'label': 'Haliburton, Kawartha, Pine Ridge District Health Unit', 'value': 'Haliburton, Kawartha, Pine Ridge District Health Unit'},
...
for i in data_raw['Reporting_PHU'].unique():
...
def update_figure(phu):
    ...
    layout = go.Layout(title= 'Covid-19 disease trend for '+ str(phu),
                       yaxis2= dict(title = 'Cumulative frequency', side = 'right', overlaying = 'y', color = 'red', rangemode= 'nonnegative', showgrid = False)
...
def rolling(phu):
    ...
    layout2 = go.Layout(title= '14-day rolling average for '+ phu,
                  yaxis=dict(title = '14-day rolling average', color = 'blue', showgrid = True, rangemode= 'nonnegative'),
                   legend= dict(orientation = 'h')
                  )
